package vk.gui
{
  ...
  */
  public class BoxLite extends Sprite
  {
	...
	private var target_layer:Sprite;
    
    public static var STAGE_WIDTH:uint = 627;
    
    private static const BORDER_W:uint = 0;
    private static const TITLE_H:uint = 0;//29;
    private static const BOTTOM_H:uint = 31;
    ...
    private static const CONTENT_Y:uint = TITLE_H + 3; //+14;
    private static const BOTTOM_DY:uint = 5;//27;
	
	private static const MIN_BUTTON_W:uint = 60;//27;
	
    
    public function BoxLite( content:*, y:int, w_:uint, buttons:Array, target_el:Sprite ):void
    {
      this.w = w_;
      this.x = Math.round( (STAGE_WIDTH - w) / 2 );
      this.y = y;
      this.target_layer = target_el;
      ...
      // Background
      Utils.fillRect( this, -BORDER_W, -BORDER_W, w + 2 * BORDER_W, h + 2 * BORDER_W, 0xCAD1D9, 0.5 );
	  ...
	  var buttons_cont:Sprite = new Sprite();
      var xx:int = 0;
      var yy:int = h - BOTTOM_H + 3;// + 8;
      for ( var i:int = 0; i < buttons.length; i++ ){
        var but:* = new SquareButton( buttons[i], xx, 0, (i==0) ? Button.BLUE_BUTTON : Button.GRAY_BUTTON );
        if (MIN_BUTTON_W > 0 && but.width < MIN_BUTTON_W)  but.width = MIN_BUTTON_W;
		but.x = xx;
        xx = but.width + 10;
        buttons_cont.addChild( but );
        but.idx = i;
        but.addEventListener( MouseEvent.CLICK, onBut );
      }	
	  buttons_cont.y = yy;
	  buttons_cont.x = (w - buttons_cont.width) / 2;
	  addChild(buttons_cont);
    ...
    public function setVisible( vis:Boolean ):void
    {
      ...
      if ( vis )
        ...
        MouseWheel.release();
	  if (target_layer){
		target_layer.filters = vis?[new BlurFilter(3, 3, BitmapFilterQuality.HIGH)]:[];
	  }
    }
