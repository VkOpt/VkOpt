...
function vkPhotoUrlUpload(url){
	...
	dApi.call('photos.getAlbumsCount',{},function(){
      ...
      "lang.loading_info":IDL('puLoadingInfoWait')
			//,"lang.button_upload":'«агрузить фотографию'
		};
...
function vkAudioNode(node){
  ...
  var makedownload=function(url,el,id,name){
    ...
    td.innerHTML='<a href="'+url+'"  download="'+name+'" onclick="return vkDownloadFile(this);" onmouseover="vkDragOutFile(this);"><div class="play_new down_btn" id="down'+id+'"></div></a>';//<img src="'+icon_src+'">
    tr.appendChild(td);  
    el.setAttribute('vk_ok','1');  
    //vk$(this).dragout();
  }
...
function vkDownloadPostfix(){
	return '';
	/*!
	активаци€ функции контакта измен€юща€ загловок ответа, 
	дл€ скачивани€ файла мину€ плагины типа QuickTime. 
	но есть веро€тность оказатьс€ на виду у разработчиков контакта и спалитьс€ за скачиванием музыки
	*/	
	return 'dl=1';
}
...
vkLastFM={
   ...
   on_location:function(){
      ...
      if (nav.objLoc['act']=="vkscrobbler" && nav.objLoc['token']){ 
         ...
         fm.auth(function(){
            vkAlertBox(fm.lang['AuthBoxTitle'], fm.lang['AuthDone'].replace(/%username/g,localStorage['lastfm_username']));
         });    
      }
   },
   ...
   auth:function(callback){
      ...
      fm.lastfm.auth.getSession({token: fm.token}, {success: function(data){
         fm.username = data.session.name;    // им€ пользовател€.
         ...
         if (code == 4)// токен сдох
            vkAlertBox(fm.lang['AuthBoxTitle'], fm.lang['AuthBoxText'], function(){
   ...
   logout:function(){
      var fm=vkLastFM;
      vkAlertBox(fm.lang['AuthBoxTitle'], fm.lang['LogoutSubmit'].replace(/%username/g,localStorage['lastfm_username']), function(){
   ...
   tip:function(el,text, dx, dy1, dy2){ 
            dx=dx||0;
            dy1=dy1||-13;
            dy2=dy2||-12;
   ...
   set_icon:function(audio_info,icon){
      ...
      if (!ge('vk_lastfm_status_icon')) {
         ...
         ge('vk_lastfm_icon').onmouseover=function(){
            var text=fm.lang[fm.enable_scrobbling?'ScrobblingOn':'ScrobblingOff'].replace(/%username/g,fm.username);
            text+=' <a href="#" onclick="vkLastFM.logout();">'+fm.lang['Logout']+'</a>';
            if (!fm.username) text=fm.lang['AuthNeeded'];
      ...
      switch (icon) {
         case 'playing': 
            ...
            el.onmouseover=function(){
               var text=fm.lang['TimeRemaining'].replace('%time',(new Date(fm.s_timer.get_remaing())).format('MM:ss'));
               //if (fm.scrobled) text=fm.lang['TrackPassed']
               fm.tip(el, text);
            }
            el.className='vk_lastfm_playing_icon';
            break;
         case 'paused':
            show(el);
            el.className='vk_lastfm_paused_icon';
            break; 
         case 'scrobled':
            show(el);
            el.onmouseover=function(){
               fm.tip(el, fm.lang['TrackScrobbled']);;
            }
            el.className='vk_lastfm_ok_icon';
            break; 
         case 'scrobled_fail':
            show(el);
            el.onmouseover=function(){
               fm.tip(el, fm.lang['TrackNotScrobbled']);;
            }
            el.className='vk_lastfm_fail_icon';
            break;             
         case 'hide':
            hide(el);
            el.className='';
   ...
   onLibFiles:function(file_name){
      if (file_name=='new_player.js') Inj.End('audioPlayer.setGraphics','vkLastFM.onPlayerState(act);');
...
(function(){
   ...
   .vk_lastfm_ok_icon{float:left; height:16px; width:8px; background:url("'+vkLastFM.res.scrobble_ok+'") 0 50% no-repeat;}\
   .vk_lastfm_fail_icon{float:left; height:16px; width:8px; background:url("'+vkLastFM.res.scrobble_fail+'") 0 50% no-repeat;}\
