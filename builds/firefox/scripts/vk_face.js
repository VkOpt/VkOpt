  ...
  for (var i=0;i<items.length;i++) if (items[i].parentNode.tagName=='LI'){
    ...
    if (submenu && exm){
      ...
      for (var k=0;(submenu && k<submenu.length);k++){
          ...
          html+=(submenu[k][2] && bcout)?'<li><a '+href+onclick+'>- '+submenu[k][1]+'</a></li>':'';/*.replace(/%%/i,bcout)  href="'+submenu[k][0]+'"*/
          html+=(!submenu[k][2])?'<li><a '+href+onclick+'>- '+submenu[k][1]+'</a></li>':'';
...
function UpdateCounters(only_msg,data){	
	...
	var menu_vars = { 
		...
		"events": 	{i:6, id:'ev',lnk:'events',add:'tab=invitations'}
	...
	if (!window.VK_LAST_COUNTERS) {
		...
		if (!only_msg){
			for (var key in menu_vars){
				...
				var el=ge('l_'+e.id);
				if (!el) return;
				var v=geByTag1('span', el.firstChild).innerHTML.match(/\d+/);
				v=v?v:0;
				cnt[e.i]=v;
			}
...
function vkClock() {
	if (getSet(30) > 0) {
		if (getSet(30) < 3) {
			var sidebar=sideBar();
			var div=vkCe('div',{id:"vkCl","class":"leftAd left_box",style:"color: #2b587a; font-size: 22px; font-family: arial; font-weight: bold;"},new Date().toLocaleString().match(/\d+:\d+:\d+/i));
